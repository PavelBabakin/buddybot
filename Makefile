...
build: format get
	...
	CGO_ENABLED=0 GOOS=$(detected_OS) GOARCH=$(detected_arch) go build -v -o buddybot -ldflags "-X="github.com/PavelBabakin/buddybot/cmd.appVersion=${VERSION}

linux: format get
	@printf "$GTarget OS/ARCH: $Rlinux/$(detected_arch)$D\n"
	CGO_ENABLED=0 GOOS=linux GOARCH=$(detected_arch) go build -v -o buddybot -ldflags "-X="github.com/PavelBabakin/buddybot/cmd.appVersion=${VERSION}
...
windows: format get
	...
	CGO_ENABLED=0 GOOS=windows GOARCH=$(detected_arch) go build -v -o buddybot -ldflags "-X="github.com/PavelBabakin/buddybot/cmd.appVersion=${VERSION}
...
darwin:format get
	...
	CGO_ENABLED=0 GOOS=darwin GOARCH=$(detected_arch) go build -v -o buddybot -ldflags "-X="github.com/PavelBabakin/buddybot/cmd.appVersion=${VERSION}
	docker build --build-arg name=darwin -t ${REGESTRY}/${APP}:${VERSION}-darwin-$(detected_arch) .

arm: format get
	@printf "$GTarget OS/ARCH: $R$(detected_OS)/arm$D\n"
	CGO_ENABLED=0 GOOS=$(detected_OS) GOARCH=arm go build -v -o buddybot -ldflags "-X="github.com/PavelBabakin/buddybot/cmd.appVersion=${VERSION}
